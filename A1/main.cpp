...
// load ',' separated int data from file
vector<Transaction> loadData(string filename, char delimiter, int &largestItem) {
    ...
    // factor=count*1.0/(items.size()*1.0);
    return data;
}
...
void dataPreprocessing(vector<Transaction> &data){
    // sort each transaction in increasing order and remove duplicates
...
bool isSubset(Transaction transaction, set<int> freqItemSet){
    // check if a is subset of b
    int i=0; auto it=freqItemSet.begin();
    while(i<transaction.size() && it!=freqItemSet.end()){
       if(transaction[i]==*it){ i++; it++;}
       else if(transaction[i]<*it){ i++;}
       else return false;
    }
    if(it==freqItemSet.end()) return true;
    return false;
}


void replace(Transaction &transaction, set<int> &freqItems, int &code){
    int i=0,j=0;
    auto it=freqItems.begin();
    while(i<transaction.size() && it!=freqItems.end()){
        if(transaction[i]==*it){ 
             it++;
    ...
    transaction.push_back(code);
}


vector<set<Item>> filterFPOutput(set<Pattern> &patterns){
    vector<set<int>> freqItems;
    // long sums=0;
    // for(auto it=patterns.begin();it!=patterns.end(); it++){
    //     sums+=it->second*it->first.size();
    // }
    // sums/=patterns.size();
    // cout<<"Patterns: "<<endl;
    for(auto it=patterns.begin();it!=patterns.end(); it++){
    ...
    // if(it->second*it->first.size()>sums){
        // cout<<it->second<<": ";
        // for(auto x: it->first){
        //     cout<<x<<" ";
        // } cout<<endl;
         freqItems.push_back(it->first);
       }
    }
    cout<<"==============================="<<endl;
    sort(freqItems.begin(),freqItems.end(),[](set<int> a,set<int> b){
...
class Compressor{
    private:
    // map<vector<int>,int,Compare> Map;
    map<set<int>,int> Map;
    ...
    public:
    Compressor(){}
    Compressor(vector<set<int>> freqItems, int thres){
        Set=freqItems;
        for(int i=0;i<freqItems.size();i++){
            Map[freqItems[i]]=thres+1+i;
            reverseMap[thres+1+i]=freqItems[i];
            threshold=thres;
        }
        // sort(Set.begin(),Set.end(),[])
    }

    void compress(vector<Transaction> &transactions, string filename){
        map<int,int> Hash;
        long incount=0; long outcount=0;
        for(auto &transaction: transactions){
            incount+=transaction.size();
            for(auto key: Set){
                // if(key.size()>2){
                    if(isSubset(transaction,key)){
                        replace(transaction,key,Map[key]);
                        Hash[Map[key]]++;
                    }
                // }
            }
        ...
        //     if(Hash[i]<4){
        //         Hash.erase(i);
        //         for(int x: reverseMap[i]) transaction.push_back(x);
        ...
        cout<<"Input Size:             "<<incount<<endl;
        cout<<"Compressed Size:        "<<outcount<<endl;
        long diff=incount-outcount;
        double percent = diff*100/incount;
        cout<<"Diff:                   "<<diff<<endl;
        cout<<"Percentage Compression: "<<percent<<endl;
        cout<<"-----------------------------------------------"<<endl;
     ...
     void decompress(string compressed_file, string decompressed_file){
        ...
        while(getline(file,line)){
            ...
            if(n>0){
                ...
                while (getline(ss, item, ' ')) {
                    try{
                        int x=stoi(item);
                        if(reverseMap.find(x)!=reverseMap.end()){
                            for(auto y: reverseMap[x]) tx.push_back(y);
                        }
                        else tx.push_back(x);
        ...
        file.close();
        dataPreprocessing(transactions);
        ofstream outfile(decompressed_file);
        for(auto transaction: transactions){
            for(int i:transaction) outfile<<i<<" ";
            outfile<<endl;
        }
        outfile.close();
    ...
    void storeData(vector<vector<int>> v ,string filename){
        ofstream file(filename);
        for(auto transaction: v){
            for(int i:transaction) file<<i<<" ";
            file<<endl;
        }
        file.close();
    }
...
int main(int argc, char* argv[]){
    ...
    if(type=="D"){
        ...
        compressor.decompress(file1,file2);
    }
    else if(type=="C"){
        int largestnum=0;
        vector<Transaction> data = loadData(file1, ' ', largestnum);
        // uint64_t minSupport = threshold(data);
        uint64_t minSupport;
        if(data.size()<10000) minSupport=ceil(data.size()*0.7);
        else if(data.size()<100000) minSupport=ceil(data.size()*0.05);
        else minSupport=ceil(10000);

        cout << minSupport << endl;
        FPTree fptree{ data, minSupport };
        std::set<Pattern> patterns = fptree_growth( fptree );
        vector<set<int>> frequentItems=filterFPOutput(patterns);

        Compressor compressor= Compressor(frequentItems, largestnum);
        // cout<<"Compressing........."<<endl;
        dataPreprocessing(data);
        compressor.compress(data,file2);
    }
    else{
        cout<<"Incoreect Arguments"<<endl;
    }
